...
| 67 | **`no-image-metadata` discovers images by content, not by filename extension** | Owner's decision, option B, chosen 2026-08-20 and reaffirmed 2026-08-21. `check_image_metadata` used to gate which files it even opened on `IMAGE_SUFFIXES = (".jpg", ".jpeg", ".png")` before ever reading a byte — the rule verified image content with magic bytes but trusted the filename to decide what counted as an image, so a raster committed as `.webp`, `.heic`, or a GPS-bearing JPEG simply renamed to `.webp` was never opened and passed CI silently, undermining decision 33's guarantee. The gate is now content-based: every committed file (excluding `.git`) is peeked, and only bytes that are genuinely `JPEG_SIGNATURE` or `PNG_SIGNATURE` trigger metadata inspection, regardless of extension. `favicon.ico` — live at the repo root and previously uncovered entirely — is given one extra look: an ICO beginning with the ICO container header is searched for an embedded `PNG_SIGNATURE`, and a PNG-encoded frame found that way is checked the same as a standalone PNG. A genuine BMP-encoded ICO frame carries no metadata segments of the kind this rule parses and is explicitly out of scope, recorded in the function's own docstring rather than silently skipped. **Rejected:** widening `IMAGE_SUFFIXES` to add `.webp`/`.heic`/`.ico` — the existing parsers understand only JPEG and PNG byte streams, and the rule fails closed on anything unparseable, so adding those extensions without a matching parser would fail CI on every legitimate file of that type; this was tried and reconfirmed wrong on 2026-08-21. `test-design-rules.py`'s `ImageMetadataTest` pins the renamed-image catch, the embedded-PNG-in-ICO case, and that ordinary non-image repository files produce zero false positives now that the suffix gate is gone |
